function [core_comm_idx,core_comm_size] = AutoFitCommunityFraction(cluster_idx,idx_knn,opts,W_smooth,lag_mat,lags)
    ...
    for cur_lvl = 1:numel(opts.clust_community_fraction) 
        [core_comm_idx, ~] = CoreCommunity(cluster_idx,idx_knn,opts.clust_community_fraction(cur_lvl)); 
        ...
        switch opts.autofit_method
            case 'autocorrelation' %compute autocorrelation to each motif in cluster
                %problem here is that this may select again dynamicness (if everything is shifted slightly then the best will be a blob in the middle of them)
            ...
            case 'stvar' %spatiotemporal variance
                ...
                critfun = @(x) min(x,[],2);
            otherwise
                error('unknown autofit community detection method');
        end %end criteria switch
    end %the the community level loop 
    [~,idx] = critfun(criteria);
